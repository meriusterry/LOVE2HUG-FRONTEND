...
import { Visibility, LocalShipping, CheckCircle, CreditCard, AccountBalance, AttachMoney } from '@mui/icons-material';
...
const AdminOrders = () => {
  ...
  const getPaymentMethodIcon = (method) => {
    switch(method) {
      ...
      case 'eft': return <AccountBalance className="w-4 h-4" />;
      case 'cash': return <AttachMoney className="w-4 h-4" />;
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-9 gap-4 mb-8">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-4">
            <p className="text-gray-500 text-xs">Order Received</p>
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
          <div className="p-6 border-b border-gray-200">
            <div className="flex flex-wrap gap-3 mb-4">
              <button onClick={() => setFilter('all')} className={`px-4 py-2 rounded-lg transition-colors ${filter === 'all' ? 'bg-primary-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>All Orders ({stats.total})</button>
              <button onClick={() => setFilter('pending')} className={`px-4 py-2 rounded-lg transition-colors ${filter === 'pending' ? 'bg-primary-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>Pending ({stats.pending})</button>
              <button onClick={() => setFilter('order_received')} className={`px-4 py-2 rounded-lg transition-colors ${filter === 'order_received' ? 'bg-primary-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>Order Received ({stats.orderReceived})</button>
              <button onClick={() => setFilter('processing')} className={`px-4 py-2 rounded-lg transition-colors ${filter === 'processing' ? 'bg-primary-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>Processing ({stats.processing})</button>
              <button onClick={() => setFilter('shipped')} className={`px-4 py-2 rounded-lg transition-colors ${filter === 'shipped' ? 'bg-primary-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>Shipped ({stats.shipped})</button>
              <button onClick={() => setFilter('delivered')} className={`px-4 py-2 rounded-lg transition-colors ${filter === 'delivered' ? 'bg-primary-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>Delivered ({stats.delivered})</button>
            </div>
            
            <div className="flex flex-wrap gap-3 pt-3 border-t border-gray-100">
              <span className="text-sm text-gray-500 font-medium">Payment Status:</span>
              <button onClick={() => setStatusFilter('all')} className={`px-3 py-1 rounded-lg text-sm transition-colors ${statusFilter === 'all' ? 'bg-primary-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>All</button>
              <button onClick={() => setStatusFilter('paid')} className={`px-3 py-1 rounded-lg text-sm transition-colors ${statusFilter === 'paid' ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>Paid</button>
              <button onClick={() => setStatusFilter('pending')} className={`px-3 py-1 rounded-lg text-sm transition-colors ${statusFilter === 'pending' ? 'bg-yellow-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>Pending</button>
              <button onClick={() => setStatusFilter('failed')} className={`px-3 py-1 rounded-lg text-sm transition-colors ${statusFilter === 'failed' ? 'bg-red-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>Failed</button>
            </div>
          </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-600">{new Date(order.created_at).toLocaleDateString('en-ZA')}</td>
                    <td className="px-6 py-4 text-sm text-gray-600">{order.item_count || order.items?.length || 0} items</td>
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900">R{parseFloat(order.total).toFixed(2)}</td>
                    ...
                    <td className="px-6 py-4">
                      <button onClick={() => { setSelectedOrder(order); setIsDetailOpen(true); }} className="text-primary-500 hover:text-primary-600">
