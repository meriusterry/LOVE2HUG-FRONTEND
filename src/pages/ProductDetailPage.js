...
const ProductDetailPage = () => {
  ...
  const getSizeOptions = () => {
    ...
    if (product.category === 'Giant') {
      ...
      return [
        { label: 'Standard', price: basePrice, inStock: product.stock > 0 },
      ];
    }
  };

  const defaultFeatures = [
    '100% premium soft plush material',
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="container mx-auto px-4">
        <div className="mb-6">
          <button onClick={() => navigate('/shop')} className="text-primary-500 hover:text-primary-600 transition-colors">
            ← Back to Shop
          </button>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-12">
            ...
            <div className="flex gap-4 mt-4">
              {productImages.map((img, idx) => (
                <div 
                  ...
                  className={`w-24 h-24 rounded-lg overflow-hidden cursor-pointer border-2 transition-all duration-300 ${
                    selectedImage === idx ? 'border-primary-500 shadow-lg' : 'border-transparent hover:border-primary-300'
                  }`}
                >
          ...
          >
            {product.badge && (
              <div className="inline-block bg-primary-500 text-white px-3 py-1 rounded-full text-sm font-semibold mb-3">
                {product.badge}
              </div>
            )}
            ...
            <div className="flex items-center gap-4 mb-4">
              <div className="flex items-center">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} className={`w-5 h-5 ${i < Math.floor(parseFloat(product.rating) || 0) ? 'text-yellow-400' : 'text-gray-300'}`} />
                ))}
                <span className="ml-2 text-gray-600">{parseFloat(product.rating).toFixed(1)} ({product.reviews_count || 0} reviews)</span>
              </div>
            </div>
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-3">
                  {sizeOptions.map((size) => (
                    ...
                    >
                      {size.label} - R{parseFloat(size.price).toFixed(2)}
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className="mb-4">
              <span className={`text-sm font-semibold ${parseInt(product.stock) > 0 ? 'text-green-600' : 'text-red-600'}`}>
                {parseInt(product.stock) > 0 ? `✓ In Stock (${product.stock} units available)` : '✗ Out of Stock'}
              </span>
            </div>
            ...
            <div className="flex gap-4 mb-8">
              <button 
                ...
                className={`flex-1 btn-primary inline-flex items-center justify-center gap-2 ${
                  parseInt(product.stock) === 0 ? 'opacity-50 cursor-not-allowed' : ''
                }`}
              >
                <ShoppingCart /> {parseInt(product.stock) > 0 ? 'Add to Cart' : 'Out of Stock'}
              </button>
              <button className="w-12 h-12 rounded-full border border-gray-300 flex items-center justify-center hover:bg-pink-50 transition-colors">
            ...
            <div className="grid grid-cols-3 gap-4 mt-8 pt-6 border-t border-gray-200">
              <div className="text-center">
                <LocalShipping className="text-primary-500 w-8 h-8 mx-auto mb-2" />
                <p className="text-sm text-gray-600">Free Shipping</p>
                <p className="text-xs text-gray-400">On orders over R500</p>
              </div>
              <div className="text-center">
                <Security className="text-primary-500 w-8 h-8 mx-auto mb-2" />
                <p className="text-sm text-gray-600">Secure Payment</p>
                <p className="text-xs text-gray-400">100% protected</p>
              </div>
              <div className="text-center">
                <Cached className="text-primary-500 w-8 h-8 mx-auto mb-2" />
